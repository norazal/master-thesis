...
# [2] Import of data 
# Read tsv-file "product_absence_filtered.tsv"
wide_product <- fread("C:/A-Benutzerdateien/BOKU/5. Semester/Master Thesis (Cronobacter)/Tabellen (2025)/RStudio gff_filter/product_absence_filtered.tsv", sep = "\t", header = TRUE)
product_df <- as.data.frame(wide_product)

# [2.1] Exclusion of column "source_file" and columns with column sum below certain threshold (10%)
# Figure out, which columns exceed the threshold
# col_sum <- colSums(wide_product[ , -1])
# threshold <- 0.1 * 3990
# filtered_columns <- names(col_sum[col_sum > threshold])

# df_filtered <- product_df[ , c(filtered_columns)]
...
# [4] Calculation of Jaccard-distance (takes approx. 10 minutes)
jc_dist_product_all <- vegdist(df_filtered, method = "jaccard")

# Checking Jaccard matrix
print(head(as.matrix(jc_dist_product_all))[1:5, 1:5])

# [5] PCoA with cmdscale (approx. 30 minutes)
pcoa_cmd_product_all <- cmdscale(jc_dist_product_all, k=2, eig = TRUE, add = TRUE)

positions <- pcoa_cmd_product_all$points
colnames(positions) <- c("PCoA1", "PCoA2")

# Calculating explained variances
percent_explained <- 100 * pcoa_cmd_product_all$eig / sum(pcoa_cmd_product_all$eig)
...
pcoa_for_plot_product_jc <- left_join(pcoa_scores, f_data, by = "Number")
...
p <- ggplot(pcoa_for_plot_product_jc, aes(x = PCoA1, y = PCoA2)) +
  ...
  labs(
    ...
    subtitle = "Cronobacter spp. - Product",
...
print(p)
ggsave("C:/A-Benutzerdateien/BOKU/5. Semester/Master Thesis (Cronobacter)/Tabellen (2025)/RStudio PCoA/plot_crspp_pcoa_jac_product_all.png", width = 16, height = 8, dpi = 300)
