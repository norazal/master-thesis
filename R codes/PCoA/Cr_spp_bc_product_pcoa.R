...
# [2] Import of data 
# Read tsv-file "wide_product.tsv"
wide_product <- fread("C:/A-Benutzerdateien/BOKU/5. Semester/Master Thesis (Cronobacter)/Tabellen (2025)/RStudio gff_filter/product_filtered.tsv", sep = "\t", header = TRUE)
...
# df_filtered <- product_df[, -1][, colSums(product_df != 0) > 0.1 * 3990]

# [2.1 alternate] Exclusion of the column "source_file"
df_filtered <- product_df[, -1]
...
bc_dist_product_all <- vegdist(df_filtered, method = "bray")

# Checking Bray-Curtis-distance matrix
print(head(as.matrix(bc_dist_product_all))[1:5, 1:5])

# [5] PCoA with cmdscale (approx. 30 minutes)
pcoa_cmd_product_all <- cmdscale(bc_dist_product_all, k=2, eig = TRUE, add = TRUE)
...
pcoa_for_plot_product <- left_join(pcoa_scores, f_data, by = "Number")
...
p <- ggplot(pcoa_for_plot_product, aes(x = PCoA1, y = PCoA2)) +
...
print(p)
ggsave("C:/A-Benutzerdateien/BOKU/5. Semester/Master Thesis (Cronobacter)/Tabellen (2025)/RStudio PCoA/plot_crspp_pcoa_bc_product_all.png", width = 16, height = 8, dpi = 300)
